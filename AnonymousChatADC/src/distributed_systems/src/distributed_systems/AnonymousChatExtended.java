...
public class AnonymousChatExtended implements AnonymousChat{
	...
	public boolean createRoom(String _room_name) {
		...
		try {
			...
			if(fg.isSuccess() && fg.isEmpty())
			{
				...
				this.sendMessage(_room_name, "I am the builder of ["+ usernames.get(_room_name) + "] room.");
	...
	public boolean joinRoom(String _room_name) {
		...
		if(this.checkRooms(_room_name) != null)
			...
			if(fg.isSuccess()) {
					...
					// check if the selected room is present in the list, if isEmpty returns true, so the room is not present
					if(fg.isEmpty())
						return false;
					
					HashSet<PeerAddress> available_peers;
					available_peers = (HashSet<PeerAddress>) fg.dataMap().values().iterator().next().object();
					available_peers.add(dht.peer().peerAddress());
					
					dht.put(Number160.createHash(_room_name)).data(new Data(available_peers)).start().awaitUninterruptibly();
	...
	private String generateNickname() {
		...
		// select randomly one char from alphabet by setting the numeric range among 99 and 127 
		username = ((char)(rnd.nextInt(99)+127)) + "" + ((char)(rnd.nextInt(99)+127)) + "_" + rnd.nextInt(10000)+"-" + rnd.nextInt(99999);
